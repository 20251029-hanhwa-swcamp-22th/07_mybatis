...
import com.mycompany.transaction.domain.Order;
import com.mycompany.transaction.domain.OrderMenu;
import com.mycompany.transaction.dto.OrderDTO;
import com.mycompany.transaction.dto.OrderMenuDTO;
import com.mycompany.transaction.mapper.OrderMapper;
...
public class OrderService {
  ...
  /* Spring은 기본적으로 모든 예외를 Unchecked로 변환 */
  @Transactional(
      isolation = Isolation.DEFAULT,  // 하나의 트랜잭션으로 처리
      propagation = Propagation.REQUIRED, // 트랜잭션이 있으면 참여, 없으면 생성
      rollbackFor = Exception.class // 발생한 예외 종류 관계없이 모두 롤백, 기본은 runtimeException
  ) // 선언적 트랜잭션 처리 어노테이션
  public void registerOrder(OrderDTO orderDTO, List<OrderMenuDTO> orderMenuDTOs){
    ...
    * - 테이블과 같은 모양으로 데이터를 취급하는 객체 */
    ...
    // tbl_order 테이블의 삽입된 order_code 번호 얻어오기
