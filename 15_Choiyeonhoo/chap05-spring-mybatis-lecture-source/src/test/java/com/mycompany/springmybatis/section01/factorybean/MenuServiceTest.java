package com.mycompany.springmybatis.section01.factorybean;
...
class MenuServiceTest {
  ...
  @Mock // 모의 객체로 생성
  private SqlSessionTemplate sqlSession; // 실제 DB 연결 없이도 메서드 호출
  ...
  @BeforeEach // 테스트 메서드 매번 수행되기 전에 실행
  public void setUp(){
    Mockito.when(sqlSession.getMapper(MenuMapper.class)) // sqlSession에서 MenuMapper를 불러올때마다.
           .thenReturn(menuMapper);                     // menuMapper를 반환함
  }
  ...
  void findAllMenuByOrderableStatus_Y() {
    ...
    // 모의 객체 menuMapper 메서드 수행 시 originalList가 반환되도록 설정
    // (originalList가 DB 조회 결과라고 생각)
    Mockito.when(menuMapper.findAllMenuByOrderableStatus(orderableStatus))
