...
/* 단위 테스트
*  - Mockito를 활용하여 의존성을 모킹(Moking) 후
...
class MenuServiceTest {
  ...
  @Mock // 모의 객체로 생성
  private SqlSessionTemplate sqlSession; // 실제 DB연결 없이도 메서드 호출이 가능하다

  @Mock
  private MenuMapper menuMapper; // Mybatis가 Mapper를 실제 구현하지 않아도 메서드 호출 가능하다
  ...
  void findAllMenuByOrderableStatus_Y() {
    ...
    // 모의 객체 menuMapper의 메서드 수행 시 originalList가 반환되도록 설정
